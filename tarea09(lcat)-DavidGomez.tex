...
\subsection{Punto 45}
\subsubsection{a}
\begin{logicenv}{PM1 es tautología}
    \begin{logic}
        (\exists \mathbf{v} \, \vert\, \val{v}{((\phi \lor \phi) \to \phi)} = \mathtt{F}) & Intento por contradicción\\
        \val{v}{((\phi \lor \phi) \to \phi)} = \mathtt{F} & Def.(p0)\\
        \val{v}{(\phi \lor \phi)} = \mathtt{T} \text{ y } \val{v}{\phi} = \mathtt{F} & MT 2.23($\to$)(p1)\\
        \val{v}{(\phi \lor \phi)} = \mathtt{F} & MT 2.23($\lor$)(p2)\\
        \val{v}{(\phi \lor \phi)} = \mathtt{F} \text{ y } \val{v}{(\phi \lor \phi)} = \mathtt{F} & Contradicción (p3, p2)
    \end{logic}
    Por lo que $\vDash ((\phi \lor \phi) \to \phi)$
\end{logicenv}

\begin{logicenv}{PM2 es tautología}
    \begin{logic}
        (\exists \mathbf{v} \, \vert\, \val{v}{(\phi \to (\phi \lor \psi))} = \mathtt{F}) & Intento por contradicción\\
        \val{v}{(\phi \to (\phi \lor \psi))} = \mathtt{F} & Def.(p0)\\
        \val{v}{\phi} = \mathtt{T} \text{ y } \val{v}{(\phi \lor \psi)} = \mathtt{F} & MT 2.23($\to$)(p1)\\
        \val{v}{\phi} = \mathtt{F} \text{ y } \val{v}{\psi} = \mathtt{F} & MT 2.23($\lor$)(p2)\\
        \val{v}{\phi} = \mathtt{F} \text{ y } \val{v}{\phi} = \mathtt{T} & Contradicción (p3, p2)
    \end{logic}
    Por lo que $\vDash (\phi \to (\phi \lor \psi))$
\end{logicenv}
\begin{logicenv}{PM3 es tautología}
    \begin{logic}
        (\exists \mathbf{v} \, \vert\, \val{v}{((\phi \to \psi) \to ((\tau \lor \phi) \to (\tau \lor \psi)))} = \mathtt{F}) & Intento por contradicción\\
        \val{v}{((\phi \to \psi) \to ((\tau \lor \phi) \to (\tau \lor \psi)))} = \mathtt{F} & Def.(p0)\\
        \val{v}{(\phi \to \psi)} = \mathtt{T} \text{ y } \val{v}{((\tau \lor \phi) \to (\tau \lor \psi))} = \mathtt{F} & MT 2.23($\to$)(p0)\\
        \val{v}{(\tau \lor \phi)} = \mathtt{T} \text{ y } \val{v}{(\tau \lor \psi)} = \mathtt{F} & MT 2.23($\to$)(p2)\\
        \val{v}{\tau} = \mathtt{F} \land \val{v}{\psi} = \mathtt{F} & MT 2.23($\lor$)(p3)\\
        \val{v}{\phi} = \mathtt{T} & MT 2.23($\lor$)(p4, p3)\\
        \val{v}{\phi} = \mathtt{F} & MT 2.23($\to$)(p4, p2)\\
        \val{v}{\phi} = \mathtt{F} \text{ y } \val{v}{\phi} = \mathtt{T} & Contradicción(p6, p5)
    \end{logic}
    Por lo que $\vDash ((\phi \to \psi) \to ((\tau \lor \phi) \to (\tau \lor \psi)))$
\end{logicenv}

\begin{logicenv}{PM4 es tautología}
    \begin{logic}
        (\exists \mathbf{v} \, \vert\, \val{v}{((\phi \lor \psi) \to (\psi \lor \phi))} = \mathtt{F}) & Intento por contradicción\\
        \val{v}{((\phi \lor \psi) \to (\psi \lor \phi))} = \mathtt{F} & Def.(p0)\\
        \val{v}{(\phi \lor \psi)} = \mathtt{T} \text{ y } \val{v}{(\psi \lor \phi)} = \mathtt{F} & MT 2.23($\to$)(p1)\\
        \val{v}{\psi} = \mathtt{F} \text{ y } \val{v}{\phi} = \mathtt{F} & MT 2.23($\lor$)(p2)\\
        \val{v}{(\phi \lor \psi)} = \mathtt{F} & MT 2.23($\lor$)(p3)\\
        \val{v}{(\phi \lor \psi)} = \mathtt{T} \text{ y } \val{v}{(\phi \lor \psi)} = \mathtt{F} & Contradicción (p4, p2)
    \end{logic}
\end{logicenv}
